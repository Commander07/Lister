...
https://github.com/SzieberthAdam/jsonfile
"""
...
class JSONFileBase:
    ...
    def _inputvalue(value):
        if isinstance(value, (collections.abc.Mapping, JSONFileObject)):
            return {
                str(k): JSONFileBase._inputvalue(v) for k, v in value.items()
            }  # ensure string keys
...
class JSONFileRoot(JSONFileBase):
    ...
    def __init__(
        self, path, *, data=..., autosave=True, dump_kwargs=None, load_kwargs=None
    ):
        self._jsonfile = self
        self._data = data
        self.autosave = autosave
        self.dump_kwargs = (
            self.default_dump_kwargs if dump_kwargs is None else dump_kwargs
        )
        self.load_kwargs = (
            self.default_load_kwargs if load_kwargs is None else load_kwargs
        )
        self.path = path
        if data is ...:
    ...
    def save(self, *, ensure_parents=True):
        ...
        with tempfile.NamedTemporaryFile(
            "w", dir=p.parent, delete=False, encoding="utf8"
        ) as tf:
            tf.write(s)
            tp = p.parent / tf.name
        tp.replace(p)
...
class JSONFileArray(JSONFileContainer, list):
    ...
    def __dir__(self):
        ...
        names.remove(
            "__add__"
        )  # JSONFileObject instances are not meant to be created directly
